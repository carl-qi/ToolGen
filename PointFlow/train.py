...
def main_worker(gpu, save_dir, ngpus_per_node, args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for bidx, data in enumerate(train_loader):
            idx_batch, tr_batch, tr_label = data['idx'], data['train_points'], data['train_label']
            ...
            input_label = tr_label.cuda(args.gpu, non_blocking=True)
            out = model(inputs, input_label, optimizer, step, writer)
        ...
        # save visualizations
        if (epoch + 1) % args.viz_freq == 0 and inputs is not None:
            # reconstructions
            model.eval()
            samples = model.reconstruct(inputs, num_points=input_label.size(1))
            results = []
            for idx in range(min(10, inputs.size(0))):
                res = visualize_point_clouds(samples[idx], input_label[idx], idx, args.dataset_type,
