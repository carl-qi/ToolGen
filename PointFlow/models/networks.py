...
class PointFlow(nn.Module):
    ...
    def forward(self, x, x_out, opt, step, writer=None):
        opt.zero_grad()
        batch_size = x.size(0)
        num_points = x_out.size(1)
        ...
        y, delta_log_py = self.point_cnf(x_out, z_new, torch.zeros(batch_size, num_points, 1).to(x))
