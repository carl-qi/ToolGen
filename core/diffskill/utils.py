...
from plb.utils.visualization_utils import save_numpy_as_gif, make_grid, save_rgb
...
def segment_partial_pcl(args, buffer, key, light, dark, view, proj):
    max_pts = {'tool_pcl': 100,
               'dough_pcl': 1000}
    max_pt = max_pts[key]
    if key in buffer.buffer.keys():
        ...
        for i in range(buffer.cur_size):
            ...
            if len(pcl > 0):
                pcl = resample_pc(pcl, max_pt)
                if 'CutRearrangeSpread' in args.env_name and key == 'tool_pcl':
                    pcl = pcl[np.where(pcl[:, 2] > 0.25)]
                buffer.buffer[key][i, :pcl.shape[0]] = pcl
            else:
                print("found an observation with zero segmented pointcloud, should prob increase resolution")
...
def aggregate_traj_info(trajs, prefix='info_'):
    ...
    for key in trajs[0].keys():
        if prefix is None or key.startswith(prefix):
            ...
            if key == 'info_final_normalized_performance':
...
def get_obs_goal_match_buffer(buffer):
    from scipy.spatial.distance import cdist
    from scipy.optimize import linear_sum_assignment
    from tqdm import tqdm
    num_goals = len(buffer.np_target_pc)
    matched_goals = np.zeros((num_goals, 1000, 3))
    for i in tqdm(range(buffer.cur_size // buffer.horizon)):
        obs = buffer.buffer['states'][i*buffer.horizon, :3000].reshape(1000, 3)
        target_v = buffer.buffer['target_v'][i*buffer.horizon]
        goal = buffer.np_target_pc[target_v]
        d = cdist(obs, goal)
        assignment = linear_sum_assignment(d)
        matched_goal = goal[assignment[1]]
        matched_goals[target_v, :, :] = matched_goal
    return matched_goals

def get_obs_goal_match(obs, goal):
    from scipy.spatial.distance import cdist
    from scipy.optimize import linear_sum_assignment
    d = cdist(obs, goal)
    assignment = linear_sum_assignment(d)
...
def visualize_trajs(trajs, ncol, key, save_name, overlay_target=False, vis_target=False, demo_obses=None, clean=False):
    """overlay_target: Whether to overlay the target images. demo_obses: whether show original demonstration on the side """
    ...
    for i in range(len(trajs)):
        ...
        for j in range(horizon):
                ...
                if key is not None:
                    if not clean and j < len(trajs[i][key]):
                        if img.shape[0] == 256:
                            ...
                            write_number(img, float(trajs[i][key][j]), fontScale=0.8, lineType=3, loc=(40, 120))
                        else:
            ...
            if demo_obses is not None:
                ...
                combined_img = np.concatenate([img, trajs[i]['target_img'][:, :, :3]], axis=0)
                imgs.append(combined_img)
            else:
                imgs.append(img)
...
def visualize_dataset(demo_path, cached_state_path, save_name, overlay_target=None, visualize_reset=False, vis_target=False, num_moves=1):
    ...
    for i in range(N):
        ...
        # if i % 8 != 0:
        imgs = []
        if vis_target or overlay_target:
        ...
        for move in range(num_moves):
            for j in range(max_step):
                ...
                num = float(buffer.buffer['info_normalized_performance'][i * horizon + max_step * move + j])
                if vis_target:
                    write_number(img, num, fontScale=0.8, lineType=3, loc=(40, 120))
                    combined_img = np.concatenate([img, target_img], axis=0)
                    imgs.append(combined_img)
                else:
                    if overlay_target:
...
def calculate_performance(buffer_path, max_step=50, num_moves=1):
    ...
    return np.mean(final_emds), np.std(final_emds)
