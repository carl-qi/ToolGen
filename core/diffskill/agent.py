...
from core.diffskill.agents.pointnet_actor import PointActor, PointActorCat
from core.diffskill.utils import batch_pred, dict_add_prefix, sample_n_ball, softclipping
from core.diffskill.agents.pointflow_vae import PointFlowVAE
from core.diffskill.agents.mlp_actor import ActorV0, ActorV1, ActorV2, ActorV3
from core.diffskill.diffskill_rgb.rgb_actor import RGBActor
...
class Agent(object):
    def __init__(self, args, solver, num_tools, device='cuda'):
        ...
        if args.input_mode == 'rgbd':
            self.dimu = self.dimz
            from core.diffskill.diffskill_rgb.vae import VAE
            self.vae = VAE(args, image_channels=len(args.img_mode), dimz=args.dimz, vae_beta=args.vae_beta).to(device)
            obs_shape = (args.img_size, args.img_size, 8)
            self.actors = nn.ModuleList([RGBActor(args, obs_shape,  action_dim=self.dima).to(device) for _ in range(num_tools)])
    ...
    def train_pc(self, data_batch, agent_ids=None, mode='train', epoch=None):
        ...
        for tid, agent_id in zip(tids, agent_ids):
            ...
            #     if not hasattr(self, 'total_step'):
            #         self.total_step = 0
            #
            #     # Add wandb visualization of the labels
            #     with torch.no_grad():
            #         obs, goal = self.vae.decode(torch.cat([z_obs_t, z_succ_goal_t], dim=0)).chunk(2, dim=0)
            #     imgs = torch.cat([obs, goal], dim=-1)
            #     pos_imgs = imgs[torch.where(succ_labels[tid].flatten() == 1)]
            #     neg_imgs = imgs[torch.where(succ_labels[tid].flatten() == 0)]
            #     from core.diffskill.utils import make_grid, img_to_np, save_rgb
            #     pos_imgs, neg_imgs = img_to_np(pos_imgs)[:, :, :, :3], img_to_np(neg_imgs)[:, :, :, :3]
            #     pos_imgs, neg_imgs = make_grid(pos_imgs, ncol=10, padding=5), make_grid(neg_imgs, ncol=10, padding=5)
            #
            #     pos_path = os.path.join(logger.get_dir(), f"pos_img_{tid}_{epoch}_{self.total_step}.png")
            #     neg_path = os.path.join(logger.get_dir(), f"neg_img_{tid}_{epoch}_{self.total_step}.png")
            #     save_rgb(pos_path, pos_imgs)
            #     save_rgb(neg_path, neg_imgs)
            #     self.total_step += 1
