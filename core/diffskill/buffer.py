...
from chester.logger import reset
from core.diffskill.hardcoded_eval_trajs import get_eval_traj
from core.diffskill.utils import batch_rand_int, load_target_imgs, load_target_pcs
...
class ReplayBuffer(object):
    def __init__(self, args, maxlen=int(150000), her_args=None):
        ...
        self.horizon = 50 if args is None or not hasattr(args,  'horizon') else self.args.horizon
    ...
    def get_tool_idxes(self, tid, idxes=None):
        ...
        elif self.args.num_tools == 2:
            if tid == 0:
                return idxes[np.where(action_mask[idxes, 0] > 0.5)[0]]
            elif tid == 1:
                return idxes[np.where(action_mask[idxes, 0] < 0.5)[0]]
        elif self.args.env_name == 'MultiTool-v1':
            init_vs = self.buffer['init_v']
            if tid == 0:
                return idxes[np.where(np.logical_and(init_vs[idxes] >= 800, init_vs[idxes] < 1000))[0]]
            elif tid == 1:
                return idxes[np.where(np.logical_and(init_vs[idxes] >= 1000, init_vs[idxes] < 1200))[0]]
            elif tid == 2:
                return idxes[np.where(
                    np.logical_or(
                    np.logical_and(init_vs[idxes] >= 0, init_vs[idxes] < 200),
                    np.logical_and(init_vs[idxes] >= 400, init_vs[idxes] < 600))
                    )[0]]
            elif tid == 3:
                return idxes[np.where(np.logical_and(init_vs[idxes] >= 200, init_vs[idxes] < 400))[0]]
            elif tid == 4:
                return idxes[np.where(np.logical_and(init_vs[idxes] >= 600, init_vs[idxes] < 800))[0]]
    ...
    def generate_train_eval_split(self, train_ratio=0.9, filter=False, cached_state_path=None):
        ...
        idxes = traj_idxes * self.horizon
        new_traj_idxes = np.array([traj_idxes[i] for i in range(len(traj_idxes)) if self.buffer['init_v'][idxes[i]] <200 or self.buffer['init_v'][idxes[i]]>=600])
        self.new_num_traj = new_num_traj = len(new_traj_idxes)
        num_train_traj = int(new_num_traj * train_ratio)
        assert not hasattr(self, 'train_traj_idx')
        hard_coded_inits, _ = get_eval_traj(self.args.cached_state_path)
        self.train_traj_idx = np.array([traj for traj in new_traj_idxes[:num_train_traj] if self.buffer['init_v'][traj*self.horizon] not in hard_coded_inits])
        self.eval_traj_idx = new_traj_idxes[num_train_traj:]
        # self.train_traj_idx = np.array([0, 1])
        # self.eval_traj_idx = self.train_traj_idx
        # print('init_v:', self.buffer['init_v'][0])
        # self.train_traj_idx = np.array([0, 200, 400, 600, 800, 1000])
        # self.eval_traj_idx = np.array([0, 200, 400, 600, 800, 1000])
    ...
    def add(self, traj, save_state=True):
        ...
        if 'contact_points' in self.buffer:
            self.buffer['contact_points'][idxes] = traj['contact_points'][:-1]
