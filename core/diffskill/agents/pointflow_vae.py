...
# from PointFlow.test import evaluate_gen, evaluate_recon # CD / EMD loss can have different g++ versions with Taichi / pytorch, which creates erros when imported.
from PointFlow.models.networks import PointFlow
import os, json, pickle
from PointFlow.args import get_args
from core.utils.utils import VArgs
from core.diffskill.utils import batch_pred_n, batch_pred
...
class PointFlowVAE(object):
    ...
    def reconstruct(self, x, num_points=None, truncate_std=None):
        ...
        t = torch.mean(x[:, :1000, :3], dim=1, keepdim=True)
        x[:, :2000, :3] = (x[:, :2000, :3] - t) / self.stat['std']
    ...
    def generate_cached_set_buffer(self, buffer):
        ...
        from core.utils.utils import array_to_list, list_to_array
        num_points = 1000
...
if __name__ == '__main__':
    # checkpoint_path = 'data/seuss/PointFlow/0408_liftspread/checkpoint-0.pt'
    checkpoint_path = 'data/PointFlow/0406_cutrearrange/checkpoint-4.pt'
    encoder = PointFlowVAE(None, checkpoint_path)

    sample = encoder.sample(25, 1000).detach().cpu().numpy()
    z = encoder.sample_latent(10)
