...
class PointNetEncoder(torch.nn.Module):
    ...
    def __init__(self, in_dim, out_dim=1024):
        ...
        self.sa3_module = GlobalSAModule(MLP([256 + 3, 256, 256, 512, out_dim]))
...
class PointNetEncoderCat(torch.nn.Module):
    ...
    def __init__(self, in_dim):
        ...
        self.sa1_module = SAModule(0.5, 0.2, MLP([3 + in_dim, 64, 64, 128]))
        self.sa2_module = SAModule(0.25, 0.4, MLP([128 + 3, 128, 128, 256]))
        self.sa3_module = GlobalSAModule(MLP([256 + 3, 256, 256, 512, 1024]))
...
class PointNetEncoder2(torch.nn.Module):
    ...
    def __init__(self, in_dim, out_dim=256):
        ...
        self.sa3_module = GlobalSAModule(MLP([3+in_dim, 128, 128, out_dim]))
        # self.sa3_module = GlobalSAModule(torch.nn.Identity())

    def forward(self, data, detach=False, calc_argmax=False):
        sa0_out = (data['x'], data['pos'], data['batch'])
        sa3_out = self.sa3_module(*sa0_out, calc_argmax=calc_argmax)
        x, pos, batch = sa3_out
        if detach:
            x.detach()
        return x
