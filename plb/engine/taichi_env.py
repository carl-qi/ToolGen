...
#     return loss
def create_chamfer_loss(bidirectional=True):
    import pytorch3d.loss as chamfer_loss
    return chamfer_loss.chamfer_distance
...
@ti.data_oriented
class TaichiEnv:
    def __init__(self, cfg, nn=False, loss=True, return_dist=False, loss_type='chamfer'):
        """
    ...
    #     return dist
    def update_loss_fn(self, adam_loss_type='chamfer'):
        if adam_loss_type == 'emd':
            self.loss_fn = create_emd_loss()
        elif adam_loss_type == 'chamfer':
            self.loss_fn = create_chamfer_loss()

    def compute_loss(self, idxes, observations,
                     vel_loss_weight, state_mask=None, goal_mask=None, loss_type='chamfer'):  # shape: n x (6 + k), first 3 position, 3 velocity, dist to manipulator, k: number of manipulator
        """ Loss for traj_opt"""
        loss = 0
        if not hasattr(self, 'loss_fn'):
        ...
        for i in range(len(xs)):
            if self.loss_type == 'emd':
                ...
                dist, _= self.loss_fn(target_x[i].unsqueeze(0), xs[i].unsqueeze(0))
                loss += dist
    ...
    #     return reward.item()
    def set_init_dist(self, loss_type):
        if loss_type == 'chamfer':
            self.set_init_chamfer()
        elif loss_type == 'emd':
            self.set_init_emd()

    def set_init_emd(self):
        self.init_dist = self.get_curr_emd()
        if isinstance(self.init_dist, torch.Tensor):
            self.init_dist = self.init_dist.item()

    def set_init_chamfer(self):
        self.init_dist = self.get_curr_chamfer()
        if isinstance(self.init_dist, torch.Tensor):
            self.init_dist = self.init_dist.item()
    ...
    def get_curr_emd(self):
        if self.tensor_target_x is None:
            ...
            return 0.
        if not hasattr(self, 'env_loss_fn'):
            self.env_loss_fn = create_emd_loss()
        curr_x = self.simulator.get_torch_x(0, self.device).contiguous()
        emd = self.env_loss_fn(curr_x, self.tensor_target_x)
        return emd
    
    def get_curr_chamfer(self):
        if self.tensor_target_x is None:
            print("Generating target, skipping setting emd")
            return 0.
        if not hasattr(self, 'env_loss_fn'):
            self.env_loss_fn = create_chamfer_loss()
        curr_x = self.simulator.get_torch_x(0, self.device).contiguous()
        dist, _ = self.env_loss_fn(curr_x.unsqueeze(0), self.tensor_target_x.unsqueeze(0))
        return dist

    def get_reward_and_info(self, subsample_n=-1):   ## TODO: add component matching option here
        if self.tensor_target_x is None:
            # Generating target
            return 0., {}
        else:
            ...
            curr_x = self.simulator.get_torch_x(0, self.device).contiguous()
            target_x = self.tensor_target_x
            if subsample_n != -1:
                sampled_idx = np.random.choice(curr_x.shape[0], subsample_n, replace=False)
                curr_x = curr_x[sampled_idx]
                target_x = self.tensor_target_x[sampled_idx]

            if self.loss_type == 'emd':
                dist = self.env_loss_fn(curr_x, target_x)
            elif self.loss_type == 'chamfer':
                dist, _ = self.env_loss_fn(curr_x.unsqueeze(0), target_x.unsqueeze(0))
            ...
            contact_loss = (self.contact_loss_mask * dists).sum() * 1e-3
            reward = -dist - contact_loss
            dist = dist.item()
            if not hasattr(self, 'init_dist'):
                normalized_performance = 0.
            elif self.init_dist == 0.:
                normalized_performance = 1.
            else:
                normalized_performance = (self.init_dist - dist) / self.init_dist
            info = {'info_emd': dist,
