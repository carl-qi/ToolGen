...
class Capsule(Primitive):
    ...
    def initialize(self):
        ...
        if not self.has_saved_points and self.cfg.pcl_path != '':
            self.save_tool_pcl(0)
            self.has_saved_points = True
...
class TestTool(Primitive):
    def __init__(self, **kwargs):
        super(TestTool, self).__init__(**kwargs)
        import mesh2sdf
        import trimesh
        # filename = '/home/carlqi/research/tool_and_dough/assets/KitchenKnife.obj'
        filename = self.cfg.obj_path
        # filename = 'assets/ladle/ladle.obj'

        self.mesh_scale = mesh_scale = 0.5   # note that the smaller the scale, the smaller level we need for better details. However, this gives bigger sdf field
        self.size = size = 128
        self.level = level = 2 / size
        ...
        self.sdf_size = size
        self.sdf_grid = ti.field(dtype=ti.f64, shape=(size,size,size), needs_grad=False)
        self.sdf_grid.from_numpy(sdf)

        # new way to compute sdf grid: THIS DID NOT WORK
        # from pysdf import SDF
        # f = SDF(vertices, mesh.faces)
        # x, y, z = np.meshgrid(np.linspace(-1, 1, size),np.linspace(-1, 1, size),np.linspace(-1, 1, size))
        # coordinates = list(zip(x.ravel(), y.ravel(), z.ravel()))
        # all_sdfs = f(coordinates).reshape(size,size,size)
        # self.sdf_grid = ti.field(dtype=ti.f64, shape=(size,size,size), needs_grad=False)
        # self.sdf_grid.from_numpy(all_sdfs)

    def initialize(self):
        super(TestTool, self).initialize()
        if not self.has_saved_points and self.cfg.pcl_path != '':
            self.save_tool_pcl(0)
            self.has_saved_points = True

    @ti.func
    def _sdf(self, f, grid_pos):   # This function is weird
        scale = 0.2
        grid_x = grid_pos[0] / scale
        grid_y = grid_pos[1] / scale
        grid_z = grid_pos[2] / scale

        x = ti.cast((grid_x + 1.) // (2./self.size), ti.int32)
        y = ti.cast((grid_y + 1.) // (2./self.size), ti.int32)
        z = ti.cast((grid_z + 1.) // (2./self.size), ti.int32)
        ...
        if x < 0:
            ...
            x = 0
        if x >= self.size:
            x_offset = x - (self.size -1)
            x = self.size-1
        if y < 0:
            y_offset = 0 - y
            y = 0
        if y >= self.size:
            y_offset = y - (self.size -1)
            y = self.size-1
        if z < 0:
            z_offset = 0 - z
            z = 0
        if z >= self.size:
            z_offset = z - (self.size -1)
            z = self.size-1
        
        sdf = self.sdf_grid[x, y, z]
        sdf += length(ti.Vector([x_offset*self.level, y_offset*self.level, z_offset*self.level]))
        sdf = sdf * scale
        # sdf = 1 + 1e-4 if (grid_pos[0] < -scale or 
        #             grid_pos[0] > scale or 
        #             grid_pos[1] < -scale or 
        #             grid_pos[1] > scale or 
        #             grid_pos[2] < -scale or grid_pos[2] > scale) else sdf
        return sdf

    @ti.func
    def _normal(self, f, grid_pos):
        # TODO: replace it with analytical normal later..
        d = ti.cast((1./self.size + 1e-4), ti.float32)
        n = ti.Vector.zero(self.dtype, self.dim)
        for i in ti.static(range(self.dim)):
            inc = grid_pos
            dec = grid_pos
...
class Link(Primitive):
    ...
    def initialize(self):
        super(Link, self).initialize()
    
    @ti.func
    def _sdf(self, f, grid_pos):
        q = ti.Vector([grid_pos[0], max(ti.abs(grid_pos[1]) - self.le, 0), grid_pos[2]])
        vec2 = ti.Vector([length(ti.Vector([q[0], q[1]])) - self.r1, q[2]])
        return length(vec2) - self.r2
...
class Spoon(Primitive):

    def __init__(self, **kwargs):
        super(Spoon, self).__init__(**kwargs)
        self.r = self.cfg.r
        self.h = self.cfg.h
        self.t = self.cfg.t
    
    def initialize(self):
        super(Spoon, self).initialize()
        if not self.has_saved_points and self.cfg.pcl_path != '':
            self.save_tool_pcl(0)
            self.has_saved_points = True
    ...
    @ti.func
    def _sdf(self, f, grid_pos):
        w = ti.sqrt(self.r*self.r - self.h*self.h)

        q = ti.Vector([length(ti.Vector([grid_pos[0], grid_pos[2]])), grid_pos[1]])
        a = ti.abs(length(q) - self.r) if self.h * q[0] >= w * q[1] else length(q - ti.Vector([w, self.h]))
        ret = a - self.t
        return ret
...
class Primitives:
    def __init__(self, cfgs, max_timesteps=1024):
        ...
        self.action_dims = [0]
        print("pimitives: num primitive:", len(outs))
        for i in outs:
