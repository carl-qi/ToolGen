...
class MultitaskPlasticineEnv(gym.Env):
    def __init__(self, cfg_path, version=None, nn=False, loss=True, return_dist=False, generating_cached_state=False, loss_type='chamfer',
                 tool_model_path=None):
        self.variants = None
        ...
        if tool_model_path is not None:
            assert len(cfg.PRIMITIVES) == 1
            cfg.PRIMITIVES[0]['obj_path'] = tool_model_path
            new_pcl_name = os.path.splitext(os.path.split(tool_model_path)[1])[0]
            cfg.PRIMITIVES[0]['pcl_path'] = os.path.join(cfg.ENV['pcl_dir_path'], new_pcl_name + '.npy')
        self.taichi_env = TaichiEnv(cfg, nn, loss=loss, return_dist=return_dist, loss_type=loss_type)  # build taichi environment
    ...
    def reset(self, init_v=None, target_v=None, target_cfg_modifier=None, contact_loss_mask=None):  # Init version and target version
        ...
        self.taichi_env.set_init_dist(self.loss_type)
        print('using loss type:', self.loss_type)
        print('dist after reset:', self.taichi_env.init_dist)
    ...
    def primitive_reset_to(self, idx, reset_states, thr=1e-2, img_size=64, reset_gripper2=False, return_np=True):
        ...
        while step < max_step:
            ...
            #         t_action = self.taichi_env.primitives[idx].inv_action(curr_state, reset_state, thr=5e-3)
                ...
                t_action = self.taichi_env.primitives[idx].inv_action(curr_state, reset_state, thr=1e-5)
            if t_action is None:
                break
        ...
        if return_np:
            ...
            return states, actions, obses, rs, infos
