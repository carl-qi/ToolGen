...
import torch
import numpy as np
from plb.utils.visualization_utils import save_numpy_as_gif, save_numpy_as_img
...
KEY_TO_STATUS = {
    ...
    **dict.fromkeys([ord("n"), ord("N")], (3, -1)),  # forward
    **dict.fromkeys([ord("m"), ord("M")], (3, 1)),  # backward
...
def run_sandbox():
    ...
    env = MultitaskPlasticineEnv(cfg_path=f'test_tool.yml', generating_cached_state=True)
    ...
    while True:
        ...
        try:
            ...
            if status == "quit":
                ...
                save_numpy_as_img(np.array(frames[-1, :, :, :3], os.path.join(save_dir, f'target_img.png')))
                np.save(os.path.join(save_dir, f'target_img.npy'), frames[-1, :, :, :3])
                ...
                if new_cnt <= MAX_HORIZON:
                    if tool_id ==0:
                        ...
                        action[status[0]+6*tool_id] = status[1] * VELOCITY
                    obs = frame
                    print('3:', frame.shape)
